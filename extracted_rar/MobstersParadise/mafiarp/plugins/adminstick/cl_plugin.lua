-- "gamemodes\\mafiarp\\plugins\\adminstick\\cl_plugin.lua"
...
function AdminStick:OpenAdminStickUI(isright,target)
	...
	-------------------------------------------------------------------------------------------------------------
		...
		local notes = playerInfo:AddOption( "View Notes",function() 
			RunConsoleCommand("say", "/notes "..target:SteamID())
			AdminStick.IsOpen = false
		end)
		...
		if (target:IsFrozen()) then
			...
			local freeze = playerInfo:AddOption( "Freeze",function()
				if LocalPlayer() == target then nut.util.notify("You can't freeze yourself!") return false end
	...
	-------------------------------------------------------------------------------------------------------------
		...
		local searchbank = characterInfo:AddOption( "List Owned Bank Accounts",function() 
			RunConsoleCommand("say", "/adminbankownerfind "..target:SteamID())
			AdminStick.IsOpen = false
		end)
		searchbank:SetIcon( "icon16/creditcards.png" )
		
		local searchbank1 = characterInfo:AddOption( "List Accessible Bank Accounts",function() 
			RunConsoleCommand("say", "/adminbankfind "..target:SteamID())
			AdminStick.IsOpen = false
		end)
		searchbank1:SetIcon( "icon16/creditcards.png" )
		...
		local desc = characterInfo:AddOption( "Change Description",function() 
			RunConsoleCommand("say", "/charsetdesc "..target:SteamID())
			AdminStick.IsOpen = false
		end)
		desc:SetIcon( "icon16/user_green.png" )

		local desc = characterInfo:AddOption( "Change Playermodel",function() 
			OpenPlayerModelUI(target)
		end)
		desc:SetIcon( "icon16/user_suit.png" )
		...
		if !LocalPlayer():IsUserGroup("moderator") then
			local pk = characterInfo:AddOption( "Character Ban (PK)",function() 
				Derma_Query("Are you sure you want to ban this character?", "Confirmation", "Yes", function()
			...
			local pkactive = characterInfo:AddOption( "Toggle PK On Death",function() 
				LocalPlayer():ConCommand('say /pkactive "'..target:SteamID()..'"')
				AdminStick.IsOpen = false
			end)
			pkactive:SetIcon( "icon16/lightning_go.png" )
		end
	...
	-------------------------------------------------------------------------------------------------------------
		...
		local gotoo = teleport:AddOption( "Goto",function() 
			if LocalPlayer() == target then nut.util.notify("You can't goto yourself!") return false end
		...
		local bring = teleport:AddOption( "Bring",function() 
			if LocalPlayer() == target then nut.util.notify("You can't bring yourself!") return false end
	...
	-------------------------------------------------------------------------------------------------------------
		...
		if target:IsUserGroup("donator") and !target:getNutData("petBlacklisted") then
			local blacklist = flags:AddOption( "Blacklist PET Flags",function() 
				LocalPlayer():ConCommand('say /flagblacklist "'..target:SteamID()..'"')
				AdminStick.IsOpen = false
			end)
				blacklist:SetIcon( "icon16/cross.png" )
		end
		
		local flagbearcat = flags:AddOption( "Give/Take Bearcat Flags",function() 
			LocalPlayer():ConCommand('say /flagbearcat "'..target:SteamID()..'"')
			AdminStick.IsOpen = false
		end)
		flagbearcat:SetIcon( "icon16/flag_green.png" )

		
		local flagpolice = flags:AddOption( "Give/Take Police Flags",function() 
			LocalPlayer():ConCommand('say /flagpolice "'..target:SteamID()..'"')
			AdminStick.IsOpen = false
		end)
		flagpolice:SetIcon( "icon16/flag_pink.png" )
		
		local flagpolicespecial = flags:AddOption( "Give/Take Police Special Flags",function() 
			LocalPlayer():ConCommand('say /flagpolicespecial "'..target:SteamID()..'"')
			AdminStick.IsOpen = false
		end)
		flagpolicespecial:SetIcon( "icon16/flag_purple.png" )
	-------------------------------------------------------------------------------------------------------------
