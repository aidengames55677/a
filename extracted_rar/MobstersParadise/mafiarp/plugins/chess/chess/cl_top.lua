-- "gamemodes\\mafiarp\\plugins\\chess\\chess\\cl_top.lua"
...
local function SecondsToTime(num)
	if updateTime>=86400 then
		return ("%i day%s"):format(updateTime/86400, updateTime>=172800 and "s" or "")
	elseif updateTime>=3600 then
		return ("%i hour%s"):format(updateTime/3600, updateTime>=7200 and "s" or "")
	elseif updateTime>=60 then
		return ("%i minute%s"):format(updateTime/60, updateTime>=120 and "s" or "")
	else
