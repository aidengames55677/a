...
PLUGIN.hungrySeconds = 7200 -- It takes 2 hours for the player to begin starving.
...
if (CLIENT) then
	...
	function PLUGIN:PlayerLoadedChar(client, character, lastChar)
		...
		timer.Create(timerName, 2, 0, function() 
			if (IsValid(client)) then
				...
				-- Player is starving
				if (percent <= 0) then
					client.wasStarving = true
					if (client:getMaxStamina() != 0) then
						client:setMaxStamina(client:getMaxStamina() - 1)
						if ((client.lastStarvingNotify or 0) < CurTime()) then
							client:notify("You're starving, you feel your stamina decreasing...")
							client.lastStarvingNotify = CurTime() + 29 -- notify every ~30 seconds (because timer is 2 seconds)
						end
					elseif (!client.notifiedCantRun) then
						client:notify("You don't think you'll be able to run anymore unless you eat")
						client.notifiedCantRun = true
					end
				elseif (client.wasStarving) then
					client:notify("You feel your stamina come back to you")
					client.wasStarving = nil
					client.notifiedCantRun = nil
					client:restoreMaxStamina()
				end
	...
	function PLUGIN:OnHungerTick(client, percent)
		if (percent < .4) then
			if (!client.hungerNotiWarning) then
				client.hungerNotiWarning = true

				client:notify("I feel hungry.")
			end
		end
		
		if (percent < .15) then
			if (!client.hungerNotiDanger) then
				client.hungerNotiDanger = true

				client:notify("I'm starving...")
			end
		end
	end
	...
	function PLUGIN:PlayerSpawn(client)
		if (client.refillHunger) then
			...
			client.hungerNotiWarning = false
			client.hungerNotiDanger = false
		end
	end
end
