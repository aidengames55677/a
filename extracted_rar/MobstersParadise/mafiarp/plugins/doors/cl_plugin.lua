-- "gamemodes\\mafiarp\\plugins\\doors\\cl_plugin.lua"
...
function PLUGIN:DrawEntityInfo(entity, alpha)
	if (entity.isDoor(entity) and !entity:getNetVar("hidden") and hook.Run("CanDrawDoorInfo") != false) then
		...
		local name = entity.getNetVar(entity, "title", entity.getNetVar(entity, "name", (IsValid(owner) or entity:getNetVar("owner", 0) > 0) and L"dTitleOwned" or L"dTitle"))
		...
		elseif (faction) then
			local faction_y = y + 16
			--[[for _,info in ipairs(nut.faction.indices) do
				if (bit.band(faction, 2^info.index) == 2^info.index) then
					drawText(L("dOwnedBy", L2(info.name) or info.name), x, faction_y, colorAlpha(teamGetColor(info.index), alpha), 1, 1)
					faction_y = faction_y + 16
				end
			end]]--
			local factions = string.Split(faction, ",")
			for _, fact in ipairs(factions) do
				local info = nut.faction.teams[fact]
		...
		elseif entity:getNetVar("owner", 0) > 0 then
			drawText("This door is owned.", x, y + 16, colorAlpha(color_white, alpha), 1, 1)
		else
