-- "gamemodes\\mafiarp\\plugins\\logging.lua"
...
if (SERVER) then
	...
	nut.log.addType("itemTake", function(client, ...)
		...
		return string.format("%s has picked up '%s' (#%s).", client:Name(), itemName, itemID)
	end)
	...
	nut.log.addType("itemDrop", function(client, ...)
		...
		return string.format("%s has dropped '%s' (#%s)", client:Name(), itemName, itemID)
	end)
