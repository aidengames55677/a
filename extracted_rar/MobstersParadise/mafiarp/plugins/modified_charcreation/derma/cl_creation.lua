-- "gamemodes\\mafiarp\\plugins\\modified_charcreation\\derma\\cl_creation.lua"
...
function PANEL:onStepChanged(oldStep, newStep)
	...
	if (IsValid(self:getPreviousStep())) then
		...
		self.cancel:AlphaTo(0, ANIM_SPEED)
	else
		self.prev:AlphaTo(0, ANIM_SPEED)
		self.cancel:AlphaTo(255, ANIM_SPEED)
	end
...
function PANEL:Init()
	...
	self.model.LayoutEntity = function(_, ent)
		if _.mouseLeftDown then
			ent:SetAngles( Angle( 0, _.lastAngles[2] - (_.beginMouse - (gui.MouseX() % 360)), 0 ) )
		elseif _.mouseLeftDown and !_:IsHovered() then
			_.mouseLeftDown = false
		end
	end
	self.model.OnMousePressed = function(_, keyCode)
		if keyCode == MOUSE_LEFT then
			_.lastAngles = _.Entity:GetAngles()
			_.beginMouse = gui.MouseX()
			_.mouseLeftDown = true
		end
	end
	self.model.OnMouseReleased = function(_, keyCode)
		if keyCode == MOUSE_LEFT then
			_.mouseLeftDown = false
		end
	end
