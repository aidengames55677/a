...
local function OrganizeNotices()
	...
	for k,v in ipairs(nut.noticess) do --Panels to rearrange
		local topMargin = 0
		for k2,v2 in pairs(nut.noticess) do --All panels before (calculate margin)
			if k < k2 then
...
function nut.util.notifQuery(question, option1, option2, manualDismiss, notifType, callback)
	...
	notice:SizeTo(notice:GetWide(), QUERY_NOTICE_HEIGHT, 0.2, 0, -1, function()
		...
		if notice.opt1 and IsValid(notice.opt1) then
			...
			notice.opt1:SetText(option1 .. " (F2)")
			...
			function notice.opt1:keyThink()
				if input.IsKeyDown(KEY_F2) and (CurTime() - notice.lastKey) >= 0.5 then
