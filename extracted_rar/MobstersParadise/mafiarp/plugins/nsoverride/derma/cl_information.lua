-- "gamemodes\\mafiarp\\plugins\\nsoverride\\derma\\cl_information.lua"
	...
	function PANEL:Init()
		...
		if (!suppress or (suppress and !suppress.all)) then
			...
			if (!suppress or !suppress.desc) then
				...
				self.descReset:SetFont("nutMediumLightFont")
				self.descReset:SetTall(28)
				self.descReset:SetText("Reset to Default Description")
	...
	function PANEL:setup()
		...
		if self.descReset then
			self.descReset.DoClick = function(this, w, h)
				netstream.Start("reset_description")
			end
		end
        ...
        if (self.money) then
            local balance = nut.currency.get( char:getMoney() )
            balance = balance:gsub("(%d+)", function(n) return string.Comma(n) end)
            self.money:SetText( L( "charMoney", balance ) )
        end
		...
		if (self.time) then
			local format = "%A, %d %B "..nut.config.get("f1date").." %X"
