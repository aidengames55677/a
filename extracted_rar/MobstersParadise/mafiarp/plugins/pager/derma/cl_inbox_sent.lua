-- "gamemodes\\mafiarp\\plugins\\pager\\derma\\cl_inbox_sent.lua"
...
function PANEL:SetMessages( messages, filter )
    ...
    for k, v in pairs( messages ) do
        if v.TrashedForSender ~= TRASHED_STATUS.NOT_TRASHED then continue end
        ...
        local item = vgui.Create( "Pager.Inbox.Sent.Item", self )
        item:Dock( TOP )
...
vgui.Register( "Pager.Inbox.Sent", PANEL, "DScrollPanel" )