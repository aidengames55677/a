...
hook.Add("CalcView", "AnimationRun", function(ply, pos, angles, fov)
	...
	if curPoint and lerpEnd >= CurTime() then
		...
		if beforeFadeOut and fadeEnd ~= lerpEnd then --Checking if it is time to fadeout
			fadeStart = CurTime()
			fadeEnd = lerpEnd
		elseif fadeEnd == lerpEnd then --While fading out
			local fdt = math.TimeFraction(fadeStart,fadeEnd,CurTime())
			local alpha = Lerp(fdt,0,255)
			ANIM.curAlpha = alpha
		elseif not beforeFadeOut then --Before fadingout (fadein)
			local fdt = math.TimeFraction(fadeStart,fadeEnd,CurTime())
...
function ANIM:Run(animName, restart, callback)
	if not animName then
		Error("No animation name provided") return
	end
	...
	if not restart then --Don't set new variables for nothing if restarted
		--Getting 'from' points
		local duration = 0
		local PVSPoints = {}
