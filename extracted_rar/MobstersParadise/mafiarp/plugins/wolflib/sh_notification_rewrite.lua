...
if (SERVER) then
	...
	function nut.util.notify(message, recipient, notifType)
		netstream.Start(recipient, "notify", message, notifType)
	end
	...
	function nut.util.notifyQuery(recipient, question, option1, option2, manualDismiss, notifType, callback)
		netstream.Start(recipient, "notifyQuery", question, option1, option2, manualDismiss, notifType, callback)
	end
	...
	do
		...
		function playerMeta:notify(message, type)
			nut.util.notify(message, self, type or 7)
		end
	...
	local function OrganizeNotices()
		...
		for k,v in ipairs(nut.notices) do --Panels to rearrange
			local topMargin = 0
			for k2,v2 in pairs(nut.notices) do --All panels before (calculate margin)
				if k < k2 then
	...
	function nut.util.notify(message, type)
		local notice = createNoticePanel(7.75)
		...
		notice.notifType = type or DEFAULT_NOTICE_TYPE

		-- Add the notice we made to the list.
		OrganizeNotices()

		notice:SetTall(0)
		notice:SizeTo(notice:GetWide(), NORMAL_NOTICE_HEIGHT, 0.2, 0)
	...
	function nut.util.notifQuery(question, option1, option2, manualDismiss, notifType, callback)
		if not callback or not isfunction(callback) then Error("A callback function must be specified") end --Some basic variable needy statments
		if not question or not isstring(question) then Error("A question string must be specified") end
		if not option1 then option1 = "Yes" end --Setting defaults
	...
	function meta:notify(message, notifType)
		nut.util.notify(message, notifType)
	end
