local PANEL = {}
	function PANEL:Init()
		...
		if (!suppress or (suppress and !suppress.all)) then
			...
			if (!suppress or !suppress.model) then
				...
				self.model:DockMargin(250,0,0,0)
			...
			if (!suppress or !suppress.name) then
				...
				self.name:SetTall(60)
				self.name:Dock(TOP)
				self.name:DockMargin(50, 50, -ScrW(), 100)
				self.name:SetTextColor(color_white)
				self.name:SetExpensiveShadow(1, Color(0, 0, 0, 150))
			end
	...
	function PANEL:Think()
		if input.IsMouseDown(MOUSE_LEFT) then
	        ...
	        local rxa, rxb = owner:GetPoseParameterRange( 6 );
	        local rya, ryb = owner:GetPoseParameterRange( 7 );
	    	...
	    	if (owner:CanOverrideView() and LocalPlayer():GetViewEntity() == LocalPlayer()) then
				...
				owner.camAng.p = math.Clamp(math.NormalizeAngle( owner.camAng.p + py / 50 ), -85, 85)
				owner.camAng.y = math.NormalizeAngle( owner.camAng.y - px / 50 )
