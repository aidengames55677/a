ITEM.name = "Outfit Edit"
ITEM.desc = "An edited Outfit Base."
ITEM.category = "Outfit"
ITEM.model = "models/Gibs/HGIBS.mdl"
ITEM.width = 1
ITEM.height = 1
ITEM.outfitCategory = "model"
ITEM.permit = "permit_clothing"

ITEM.pacData = {}

ITEM.replacements = {"player/zelpa", "player/suits"}
		...
		elseif (item.replacement or item.replacements) then
			...
			if (type(item.replacements) == "table") then
				if (#item.replacements == 2 and type(item.replacements[1]) == "string") then
					local modelPath = item.player:GetModel():gsub(item.replacements[1], item.replacements[2])
					modelPath = modelPath:gsub("_extended.mdl", "_closed_tie.mdl")
					
					char:setModel(modelPath)
				else
