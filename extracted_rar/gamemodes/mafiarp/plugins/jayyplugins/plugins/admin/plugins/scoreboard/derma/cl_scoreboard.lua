local PANEL = {}
	...
	local function countPlayersInStaffAndCore()
		local count = 0
		for _, player in ipairs(player.GetAll()) do
			for _, group in ipairs(staffAndCoreGroups) do
	...
	function PANEL:Init()
		...
		self.staffCountCore = self:Add("DLabel")
		self.staffCountCore:Dock(TOP)
		self.staffCountCore:SetFont("nutSmallFont")
		self.staffCountCore:SetContentAlignment(4)
		self.staffCountCore:SetTextColor(nut.config.get("colorText", color_white))
		self.staffCountCore:SetExpensiveShadow(1, color_black)
		self.staffCountCore:SizeToContentsY()
		self.staffCountCore:SetTall(self.staffCountCore:GetTall() + 16)
		self.staffCountCore.Paint = function(this, w, h)
			surface.SetDrawColor(0, 0, 0, 150)
			surface.DrawRect(0, 0, w, h)
		end
	...
	function PANEL:Think()
		if ((self.nextUpdate or 0) < CurTime()) then
			...
			self.staffCountCore:SetText("Staff Online: " .. countPlayersInStaffAndCore())
