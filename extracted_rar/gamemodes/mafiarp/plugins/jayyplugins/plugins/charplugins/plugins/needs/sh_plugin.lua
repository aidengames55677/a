...
local thirstTime = 100
local hungerTime = 100
local dmgTime = 30
local healTime = 120
...
if(SERVER) then
            ...
            if(IsValid(ply)) then
				if(ply:getChar()) then
					...
					if(thirst > 0) then					--460
						if(curTime - lastCheckThir > (thirstTime)) then
					...
					if hunger <= 25 then
						timer.Create("HungerNotification", 60, 0, function()
							if IsValid(ply) then  -- Check if the player is still valid
								if (hunger <= 25) then
									ply:notify("You are getting hungry, eat some food, comrade.")
								else
									timer.Remove("HungerNotification")
								end
							end
						end)
					end
					...
					if thirst <= 25 then
						timer.Create("ThirstNotification", 60, 0, function()
							if IsValid(ply) then  -- Check if the player is still valid
								if (thirst <= 25) then
									ply:notify("You are getting thirsty, drink some liquid, comrade.")
								else
									timer.Remove("ThirstNotification")
								end
							end
						end)
					end
					...
					if(hunger > 0) then					--540
						if(curTime - lastCheckHung > (hungerTime)) then
	...
	hook.Add("Tick", "AdminHungerThirst", function()
		for _, ply in ipairs(player.GetAll()) do
			local char = ply:getChar()
			if char and ply:Team() == FACTION_STAFF then
				char:SetHunger(100)
				char:SetThirst(100)
			end
		end
