...
if (CLIENT) then
	...
	function themis.dialog(name, model, profession, place_of_employment, dialog )
		local npc_name = name
	...
	function themis.dialogframe(name, model, profession, place_of_employment)
		self = vgui.Create("WolfFrame")
		self:SetTitle(name .. " - " .. place_of_employment .. " " ..  profession)
		self:SetPos(ScrW() * .5 - MaxW * .5, ScrH() - MaxH * 1.5)
		self:SetSize(MaxW, MaxH)
	...
	function themis.dialogbutton(text, height, doclick)
	...
	function themis.dialogtext(text, height)
	...
	function themis.dialogtextlong(text, height)
		 
	end

	function themis.npctext(name, desc)
