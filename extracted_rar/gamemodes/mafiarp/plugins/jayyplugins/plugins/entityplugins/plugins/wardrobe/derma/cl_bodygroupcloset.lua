...
function PANEL:Init()
	...
	local h, s, v = ColorToHSV( nut.config.get("color") )
	s = s - 0.25
	...
	self.finishbutton.DoClick = function()
		...
		data.groups = groups
		netstream.Start("nut_bodygroupclosetapplychanges", data)
		self:Remove()
	end
end

function PANEL:OnRemove()
	netstream.Start("onnut_bodygroupclosetclose") -- not optimal as we dont have a reference to the entity but yea
end

vgui.Register("nut_bodygroupcloset", PANEL, "DFrame")