
PLUGIN.name = "Tying"
PLUGIN.author = "JayyKashtaCodes"
PLUGIN.desc = "Adds the ability to tie players."

nut.util.include("sh_charsearch.lua")

local langkey = "english"
do
	local langTable = {
		tying = "Tying",
		unTying = "Untying",
		beingTied = "You're being tied up.",
		beingUntied = "You're being untied.",
		isTied = "This person is restrained",
	}

	table.Merge(nut.lang.stored[langkey], langTable)
end

if (SERVER) then
	function PLUGIN:PlayerLoadout(client)
		client:setNetVar("restricted")
	end
	--[[
	function PLUGIN:PlayerUse(client, entity)
		if (!client:getNetVar("restricted") and entity:IsPlayer() and entity:getNetVar("restricted") and !entity.nutBeingUnTied) then
			entity.nutBeingUnTied = true
			entity:setAction("beingUntied", 5)

			client:setAction("unTying", 5)
			client:doStaredAction(entity, function()
				entity:setRestricted(false)
				entity.nutBeingUnTied = false

				client:EmitSound("npc/roller/blade_in.wav")
			end, 5, function()
				if (IsValid(entity)) then
					entity.nutBeingUnTied = false
					entity:setAction()
				end

				if (IsValid(client)) then
					client:setAction()
				end
			end)
		end
	end

	function PlayerUntie(client, entity)
		if (!client:getNetVar("restricted") and entity:IsPlayer() and entity:getNetVar("restricted") and !entity.nutBeingUnTied) then
			entity.nutBeingUnTied = true
			entity:setAction("beingUntied", 5)

			client:setAction("unTying", 5)
			client:doStaredAction(entity, function()
				entity:setRestricted(false)
				entity.nutBeingUnTied = false

				client:EmitSound("npc/roller/blade_in.wav")
			end, 5, function()
				if (IsValid(entity)) then
					entity.nutBeingUnTied = false
					entity:setAction()
				end

				if (IsValid(client)) then
					client:setAction()
				end
			end)
		end
	end
	]]
	util.AddNetworkString("PlayerUntie")

	net.Receive("PlayerUntie", function(len, client)
		local entity = net.ReadEntity()
		PlayerUntie(client, entity)
	end)
	
	function PlayerUntie(client, entity)
		if (!client:getNetVar("restricted") and entity:IsPlayer() and entity:getNetVar("restricted") and !entity.nutBeingUnTied) then
			entity.nutBeingUnTied = true
			entity:setAction("beingUntied", 5)

			client:setAction("unTying", 5)
			client:doStaredAction(entity, function()
				entity:setRestricted(false)
				entity.nutBeingUnTied = false

				client:EmitSound("npc/roller/blade_in.wav")
			end, 5, function()
				if (IsValid(entity)) then
					entity.nutBeingUnTied = false
					entity:setAction()
				end

				if (IsValid(client)) then
					client:setAction()
				end
			end)
		end
	end
	
else
	local COLOR_TIED = Color(245, 215, 110)

	function PLUGIN:DrawCharInfo(client, character, info)
		if (client:getNetVar("restricted")) then
			info[#info + 1] = {L"isTied", COLOR_TIED}
		end
	end

	nut.playerInteract.addFunc("playerUntying", {
		nameLocalized = "playerUntying",
		callback = function(target)
			local client = LocalPlayer()
			if (!client:getNetVar("restricted") and target:IsPlayer() and target:getNetVar("restricted") and !target.nutBeingUnTied) then
				net.Start("PlayerUntie")
				net.WriteEntity(target)
				net.SendToServer()
			end
		end,
		canSee = function(target)
			return true
		end
	})
end
--[[
if (CLIENT) then
	nut.playerInteract.addFunc("playerUntying", {
		nameLocalized = "playerUntying",
		callback = function(target)
			local client = LocalPlayer()
			if (!client:getNetVar("restricted") and target:IsPlayer() and target:getNetVar("restricted") and !target.nutBeingUnTied) then
				PlayerUntie(client, target)
			end
		end,
		canSee = function(target)
			return true
		end
	})
end]]