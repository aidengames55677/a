...
function SWEP:PrimaryAttack()
	...
	if (IsValid(entity) and
		(
			(entity:isDoor() and (entity:checkDoorAccess(self.Owner) or (self.Owner:IsAdmin() and self.Owner:Team() == FACTION_staff))) or
	...
	elseif (door:IsVehicle()) then
		if (state) then
			...
			if (door.IsSimfphyscar) then
				door.IsLocked = true
				if (door.SetIsVehicleLocked) then
					door:SetIsVehicleLocked(true)
				end
			end
			self.Owner:EmitSound("doors/door_latch3.wav")
		else
