...
function PLUGIN:ShowTeam(client)
	...
	if (IsValid(entity) and entity:isDoor() and !entity:getNetVar("faction") and !entity:getNetVar("class")) then
		if (entity:checkDoorAccess(client, DOOR_TENANT)) then
			...
			if (ApartmentSystem && entity:IsApartment()) then // Apartment System By Killing Torcher - F2 Door Menu
				net.Start("ApartmentSystem_DoorsVGUI")
				net.WriteString(entity.Apartment)
				net.Send(client)
				return
			end
