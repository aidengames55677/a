...
function playerMeta:CanOverrideView()
    ...
    return nut_CVAR_THIRDPERSON:GetBool() and not IsValid(self:GetVehicle()) and isAllowed() and IsValid(self) and self:getChar() and not self:getNetVar("actAng") and not IsValid(ragdoll) and LocalPlayer():Alive()
end
...
function PLUGIN:CalcView(client, origin, angles, fov)
    ...
    if client:CanOverrideView() and LocalPlayer():GetViewEntity() == LocalPlayer() then
        ...
        view = {}
        traceData = {}
        traceData.start = client:GetPos() + client:GetViewOffset() + curAng:Up() * clmp(nut_CVAR_TP_VERT:GetInt(), 0, maxValues.height) + curAng:Right() * clmp(nut_CVAR_TP_HORI:GetInt(), -maxValues.horizontal, maxValues.horizontal) - client:GetViewOffsetDucked() * .5 * crouchFactor
        ...
        if nut_CVAR_TP_CLASSIC:GetBool() or (owner.isWepRaised and owner:isWepRaised() or (owner:KeyDown(bit.bor(IN_FORWARD, IN_BACK, IN_MOVELEFT, IN_MOVERIGHT)) and owner:GetVelocity():Length() >= 10)) then
...
function PLUGIN:PlayerButtonDown(ply, button)
    if button == KEY_F4 and IsFirstTimePredicted() then
        local toggle = GetConVar("nut_tp_enabled")
