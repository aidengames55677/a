...
function PLUGIN:ManageTies( charId, tieBodygroup )
    if not tieBodygroup then
        tieBodygroup = 0 -- Default to no tie.
    end
...
function PLUGIN:CreateEntities( ply, itemClass )
    ...
    if itemMeta.Bonemerge and #itemMeta.Bonemerge > 0 then
        ...
        for i = 1, #itemMeta.Bonemerge do
            ...
            if bonemergeData.CalculateSkinTone and ply:IsFemale() then
                for index, submaterial in next, ent:GetMaterials() do
                    ...
                    if isCultistShared or isGTAShared then
                        local isBlack = plugin.SkinTranslation[ply:GetModel()] == 1
...
function PLUGIN:ManageEntities( ply, createEntities, removeEntities, newParent )
    ...
    for itemId, itemData in next, charData.items do
        ...
        if createEntities and self.ModelCheck[ply:GetModel()] and itemData.equipped then
            itemData.entities = self:CreateEntities( ply, itemData.class )
    ...
    for partType, partData in next, charData.parts do
        ...
        if createEntities and not hiddenParts[partType] and self.ModelCheck[ply:GetModel()] then
            ...
            if partType == "hands" then
                ent:SetSkin( self.SkinTranslation[ply:GetModel()], 0 )

                if ply:IsFemale() and handsValue > 10 then
                    handsValue = 10
                end
...
function PLUGIN:Clear()
    for _, ent in ipairs( ents.FindByClass( "class C_BaseFlex" ) ) do
        if ent.BonemergeDoNotClear or not ent:IsEffectActive( EF_BONEMERGE ) then continue end
        ent:Remove()
    end
end

function PLUGIN:FullUpdate()
    -- TODO: Check if they're in F1 menu, and do not update if they are. You can remove .BonemergeDoNotClear if this is the case.
...
function PLUGIN:PlayerLoadedChar( ply, char, oldChar )
    if oldChar and self.CharacterData[oldChar:getID()] then
        self.CharacterData[oldChar:getID()] = nil
    end
...
hook.Add( "Think", "Bonemerge.Think", function()
    local nextRefresh = PLUGIN.NextRefresh
    if not nextRefresh then
        PLUGIN.NextRefresh = CurTime()
        nextRefresh = PLUGIN.NextRefresh
    end

    if nextRefresh <= CurTime() then
        PLUGIN:FullUpdate()
        PLUGIN.NextRefresh = CurTime() + 60
    end
...
hook.Add( "player_disconnect", "Bonemerge.player_disconnect", function( data )
    ...
    if not charData.items then return end
