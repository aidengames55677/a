...
ENT.Spawnable = true
ENT.AdminOnly = true
ENT.Category = "RedDawn - Econemy"

function ENT:Use(activator, caller)
    if IsValid(activator) and activator:IsPlayer() then
        -- Add 1000 NutScript money to the player's balance
        activator:getChar():giveMoney(5000)

        -- Optional: Show a notification to the player
        activator:notify("You Have Gained ₽5000.")

        -- Remove the lootbag entity after use
        self:Remove()
	end
end
...
function ENT:Draw()
	...
	local cash    = "5000"
		...
		draw.DrawText("RM"..cash, "nut_TokensFont", 0, 0, Color(0, 180, 70, 255), TEXT_ALIGN_LEFT )
		cam.End3D2D();
