-- "gamemodes\\mafiarp\\plugins\\adminstick\\entities\\weapons\\adminstick.lua"
...
function SWEP:Deploy()
	if SERVER then
		if !usergroups[self:GetOwner():GetUserGroup()] then
			self:Remove()
		end
	end
end
...
function SWEP:PrimaryAttack()
	...
	if (IsValid(target) && !target:IsPlayer()) then
		if target:IsVehicle() && IsValid(target:GetDriver()) then
...
function SWEP:SecondaryAttack()
	...
	if (IsValid(target) && target:IsPlayer() && target != LocalPlayer()) then
...
function SWEP:DrawHUD()
	...
	if (IsValid(target)) then
		...
		if (target:IsPlayer()) then
			...
			information = { "Entity", "Class: " .. target:GetClass(), "Model: " .. target:GetModel(), "Position: " .. tostring(target:GetPos()), "Angles: " .. tostring(target:GetAngles()), "Owner: " ..tostring(target:GetNW2Entity("Creator", target:GetOwner())), "EntityID: " ..target:EntIndex()}
...
function SWEP:Reload()
	...
	if (IsValid(lookingAt) && !lookingAt:IsPlayer()) then
		if lookingAt:IsVehicle() && IsValid(lookingAt:GetDriver()) then
...
function SWEP:DrawWorldModel()
	if (!usergroups[self:GetOwner():GetUserGroup()] && self:GetOwner():GetMoveType() == MOVETYPE_NOCLIP) then
		return
	end
	self:DrawModel()
end