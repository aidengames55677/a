-- "gamemodes\\mafiarp\\plugins\\cookfood\\entities\\entities\\nut_stove.lua"
...
ENT.invType = "stove"
if !nut.inventory.types[ENT.invType] then
...
if (SERVER) then
	function ENT:Initialize()
		...
		nut.item.newInv(0, self.invType, function(inventory)
			...
			hook.Add("ItemTransfered", entity, function(context)
				if context == entity then
					context:updateContents()
				end
			end)
		end)
	end
	...
	function ENT:Use(activator)
		...
		if activator:KeyDown( IN_WALK ) then
			activator:getChar():getInv():add("stove")
			self:Remove()
			activator:notify("You picked up your stove.")
	...
	function ENT:onDrawEntityInfo(alpha)
		...
		drawText("Stove", x, y, colorAlpha(configGet("color"), alpha), 1, 1, nil, alpha * 0.65)
	end
