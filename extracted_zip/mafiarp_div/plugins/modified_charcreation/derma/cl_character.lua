-- "gamemodes\\mafiarp\\plugins\\modified_charcreation\\derma\\cl_character.lua"
...
function PANEL:createTabs()
    ...
    if (hook.Run("CanPlayerCreateCharacter", LocalPlayer()) ~= false) then
        ...
        if ( nut.characters and #nut.characters == 0 ) then
            Derma_Query(
                "Hey there, we've noticed this is your first time on the server. Would you like to read a basic server introduction guide?", 
                "New Player Detected",
                "Yes",
                function()
                    gui.OpenURL( "https://divergenet.works/forums/showthread.php?tid=3423" )
                end,
                "No" 
            )
        end
