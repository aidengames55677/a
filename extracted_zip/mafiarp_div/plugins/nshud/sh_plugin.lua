...
function PLUGIN:DrawEntityInfo(entity, alpha, position)
	...
	local posOffset = entity.Crouching(entity) and OFFSET_CROUCHING or OFFSET_NORMAL
	posOffset = Vector(posOffset.x, posOffset.y, posOffset.z * entity:GetModelScale())

	position = position or toScreen(entity.GetPos(entity) + posOffset)
