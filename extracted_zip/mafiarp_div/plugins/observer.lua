-- "gamemodes\\mafiarp\\plugins\\observer.lua"
...
if (CLIENT) then
	...
	function PLUGIN:HUDPaint()
		...
		if (offDutyNoclip[userGroup] and client:GetMoveType() == MOVETYPE_NOCLIP and !client:InVehicle() and NUT_CVAR_ADMINESP:GetBool() or client:Team() == FACTION_STAFF and client:GetMoveType() == MOVETYPE_NOCLIP and !client:InVehicle() and NUT_CVAR_ADMINESP:GetBool()) then
			sx, sy = surface.ScreenWidth(), surface.ScreenHeight()

			for k, v in ipairs(player.GetAll()) do
				if (v == client) then continue end 
				if !client:CanTargetRank(v:GetUserGroup()) and v:GetNW2Bool("NoTarg") then continue end
				if !offDutyNoclip[client:GetUserGroup()] && !v:getChar() then continue end
				if v:IsUserGroup( "founder" ) then continue end
	...
	function PLUGIN:PlayerNoClip(client, state)
		...
		if (offDutyNoclip[client:GetUserGroup()] or client:Team() == FACTION_STAFF) then
			...
			if (state) then
				...
				SAdmin:AddLog( "Noclip", client:Name() .. " has entered noclip.", client:SteamID() )
			else
				...
				hook.Run("OnPlayerObserve", client, state)
				SAdmin:AddLog( "Noclip", client:Name() .. " has left noclip.", client:SteamID() )
			end
		end
	end
end
