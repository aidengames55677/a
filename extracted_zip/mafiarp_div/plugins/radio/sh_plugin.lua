-- "gamemodes\\mafiarp\\plugins\\radio\\sh_plugin.lua"
...
local langTable = {
	radioFreq = "Frequency",
	radioSubmit = "Submit",
	radioNoRadio = "You don't have a radio to adjust.",
	radioNoRadioComm = "You don't have a radio to communicate with.",
...
if (CLIENT) then
	...
	netstream.Hook("radioAdjust", function(client, freq, id, channel)
		...
		if (inv) then
			...
			if freq == "911.1" && !allowedTeams[client:getChar():getFaction()] then 
				client:notify("Only members of Emergency Services can use this frequency!")
				return false 
			end
			...
			if (item and (IsValid(ent) or item:getOwner() == client)) then
				...
				local oldChannel = item:getData("channel", 1);

				local newChannel = tonumber(channel) or 1;
				
				item:setData("freq", freq)
				item:setData("channel", newChannel)

				if !item:getData("power") then return end

				local PLUGIN = nut.plugin.list.payphones;

				local oldMapKey = oldFreq..":"..oldChannel;
				if PLUGIN.RadioCallMap[oldMapKey] then
					PLUGIN:LeaveCall(PLUGIN.RadioCallMap[oldMapKey], client);
				end

				local mapKey = freq..":"..newChannel;

				if PLUGIN.RadioCallMap[mapKey] then
					PLUGIN:JoinCall(PLUGIN.RadioCallMap[mapKey], client);
				else
					PLUGIN.RadioCallMap[mapKey] = PLUGIN:CreateCall(client, 2);
				end
			else
