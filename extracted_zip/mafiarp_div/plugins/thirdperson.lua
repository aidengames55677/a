-- "gamemodes\\mafiarp\\plugins\\thirdperson.lua"
...
if (CLIENT) then
	...
	function PLUGIN:CalcView(client, origin, angles, fov)
		...
		if (client:CanOverrideView() and LocalPlayer():GetViewEntity() == LocalPlayer()) then
            ...
            if NUT_CVAR_TP_CLASSIC:GetBool() and wep:IsValid() and wep.ArcCW and owner:KeyDown(IN_ATTACK) then
                recoilOffset:Zero()
                local ft = (SysTime() - (lst or SysTime())) * GetConVar("host_timescale"):GetFloat()
                recoilOffset = recoilOffset + (wep:GetBuff_Override("Override_RecoilDirection") or wep.RecoilDirection) * wep.RecoilAmount
                recoilOffset = recoilOffset + (wep:GetBuff_Override("Override_RecoilDirectionSide") or wep.RecoilDirectionSide) * wep.RecoilAmountSide
                owner.camAng = owner.camAng - (recoilOffset * ft * 30)
            end
