-- "gamemodes\\mafiarp\\plugins\\vendor\\cl_networking.lua"
...
addNetHandler("AllowFaction", function(vendor)
	...
	if !nut.faction.indices[id] then return end
	
	local uniqueID = nut.faction.indices[id].uniqueID

	if (allowed) then
		vendor.factions[uniqueID] = true
	else
		vendor.factions[uniqueID] = nil
	end
...
net.Receive("nutVendorFaction", function()
	...
	local uniqueID = nut.faction.indices[factionID].uniqueID

	if (IsValid(nutVendorEnt)) then
		nutVendorEnt.factions[uniqueID] = true
	end
end)
