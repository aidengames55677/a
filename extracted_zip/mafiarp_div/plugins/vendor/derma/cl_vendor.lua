-- "gamemodes\\mafiarp\\plugins\\vendor\\derma\\cl_vendor.lua"
...
function PANEL:initializeItems()
	local sortedItems = {}
	local itemNames = {} 
	for itemType in pairs(nutVendorEnt.items) do
		local item = nut.item.list[itemType]
		if item then
			table.insert(sortedItems, itemType)
			itemNames[itemType] = item.name  
		end
	end
	...
	for _, itemType in ipairs(sortedItems) do
