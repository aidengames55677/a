-- "gamemodes\\mafiarp\\plugins\\vendor\\derma\\cl_vendoreditor.lua"
...
function PANEL:Init()
	...
	self.search:DockMargin(0, 4, 0, 0)
	...
	local function List(val)
		self.lines = {}
		for k, v in SortedPairsByMemberValue(nut.item.list, "name") do
			if val && !string.find(string.lower(v.name),string.lower(val)) then continue end
