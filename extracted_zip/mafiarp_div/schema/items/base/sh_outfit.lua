-- "gamemodes\\mafiarp\\schema\\items\\base\\sh_outfit.lua"
...
ITEM.functions.Equip = {
	...
	onCanRun = function(item)
		return not IsValid(item.entity) and item:getData("equip") ~= true and nut.inventory.instances[item.invID] == item.player:getChar():getInv()
	end
}
...
function ITEM:onRemoved()
	local inv = nut.inventory.instances[self.invID]
	if (IsValid(inv) and IsValid(receiver) and receiver:IsPlayer()) then
		if (self:getData("equip")) then
