...
nut.command.add("getvotes", {
	...
	onRun = function(ply, arguments)
		local toPrint = "USSR Voting Results:"
		local total = 0
		local results = {}
...
nut.command.add("endvoting", {
	...
	onRun = function(ply, arguments)
		...
		ply:notify("Voting has now ended.")
	end
})
...
if CLIENT then
	...
	function PANEL:Init()
		...
		self.panel.label:SetText("Welcome to the USSR Voting. Please select carefully who you want to vote for as you may only vote for one person, one time.")
