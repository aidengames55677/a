NUT_ITEM_DEFAULT_FUNCTIONS = {
	drop = {
		...
		onRun = function(item)
			...
			nut.log.add(item.player, "itemDrop", item.name, item.id, 1)
	...
	take = {
		...
		onRun = function(item)
			...
			if not inventory:findFreePosition(item.uniqueID) then
				client:notify("Not enough room in your inventory!")
				return false
			end
			...
			inventory:add(item)
				:next(function(res)
					...
					nut.log.add(client, "itemTake", item.name, item.id, 1)
